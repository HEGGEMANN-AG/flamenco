...
#[derive(Clone, Debug)]
pub struct BothDirectoryInformation {
    pub file_index: u32,
...
impl QueryInformation for BothDirectoryInformation {
    fn class() -> DirectoryInformationClass {
        DirectoryInformationClass::BothDirectory
    }
    fn read_from_buffer<R: Read + Seek>(r: &mut R) -> Result<(Self, bool), std::io::Error> {
        ...
        r.read_exact(slice::from_mut(&mut short_name_length))?;
        // Reserved
        r.seek_relative(1)?;
        assert!(short_name_length <= 24);
        let mut short_name_buf = vec![0; short_name_length as usize];
        r.read_exact(&mut short_name_buf)?;
        r.seek_relative((24 - short_name_length).into())?;
        let mut name_bytes = vec![0; file_name_length as usize];
        r.read_exact(&mut name_bytes)?;
        let short_file_name: Box<str> = crate::from_wide(&short_name_buf).into_boxed_str();
